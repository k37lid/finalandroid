...
public class Syn extends AsyncTask<String,Void,String> {
    ...
    private String onInsert(String ... params){
        String phpPageULR="http://10.180.0.49:80/sqli/mysql_write.php";
        try {
            ...
            // users will start writing the information
            student.write(information);
            // users will push the information from the client side to the server side
            student.flush();
            // users finished his job
            student.close();
    ...
    private String Read(String ... params){
        String phpPageULR="http://10.180.0.49:80/sqli/mysql_read.php";
        try {
            // preparing the URL for the connection
            URL url=new URL(phpPageULR);
            // open a channel between the client(android device) and the server (PHP)
